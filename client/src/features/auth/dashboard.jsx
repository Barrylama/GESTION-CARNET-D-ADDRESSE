...
function Dashboard() {
  ...
  const {data:dataCount, isLoadingCount, isSuccessCount, isErrorCount} = useCountContactQuery();

  return (
    <div>
      {/* Section pour le nombre total d'utilisateurs */}
      <section className="py-10 m-5 flex rounded-xl" style={{
        ...
        <div className="w-1/2 mt-5 ">
          ...
          <div className="bg-white p-4 rounded-lg w-2/3 ml-10 shadow-md">
            ...
            {data?.totalUsers !== null ? (
              <p className="text-6xl text-center font-bold rounded-md pt-5 pb-5" style={{
               background: "#a8e1bd"
             }}>{data?.totalUsers}</p>
            ) : (
               <>
               <span className="mr-2">Chargement du nombre de contact en cours ......</span>
          ...
          <div className="bg-white p-4 rounded-lg w-2/3 ml-10 mt-5 shadow-md">
            <h2 className="text-3xl font-bold mb-4">Contacts enregistrés</h2>
            {dataCount?.count !== null ? (
              <p className="text-6xl text-white text-center font-bold rounded-md pt-5 pb-5" style={{
               background: "#ff6581"
             }}>{dataCount?.count}</p>
            ) : (
               <>
